...
// Temp registers used to materialize constant operands: w0/x0 for the first
// source, w1/x1 for the second. Sequence dest registers always come from the
// HIR pools (x19-x26 / v16-v31) so these never alias.
template <typename T>
static const T GetTempReg1(A64Emitter& e);
template <>
inline const WReg GetTempReg1<WReg>(A64Emitter& e) {
  return WReg(0);
}
template <>
inline const XReg GetTempReg1<XReg>(A64Emitter& e) {
  return XReg(0);
}
template <typename T>
static const T GetTempReg2(A64Emitter& e);
template <>
inline const WReg GetTempReg2<WReg>(A64Emitter& e) {
  return WReg(1);
}
template <>
inline const XReg GetTempReg2<XReg>(A64Emitter& e) {
  return XReg(1);
}
...
struct Sequence {
  ...
  // the given temp register. ARM64 sequences are three-operand, so unlike the
  // x64 versions no dest/src coalescing is needed.
  template <typename OP>
  static typename OP::reg_type GetWithConst(
      A64Emitter& e, const OP& src, const typename OP::reg_type& temp) {
    if (src.is_constant) {
      e.MovConst(temp, src.constant());
      return temp;
    }
    return src.reg();
  }
  ...
  static void EmitUnaryOp(A64Emitter& e, const EmitArgType& i,
                          const REG_FN& reg_fn) {
    auto src1 = GetWithConst(
        e, i.src1, GetTempReg1<typename decltype(i.src1)::reg_type>(e));
    reg_fn(e, i.dest, src1);
  }

  template <typename REG_REG_FN>
  static void EmitBinaryOp(A64Emitter& e, const EmitArgType& i,
                           const REG_REG_FN& reg_reg_fn) {
    auto src1 = GetWithConst(
        e, i.src1, GetTempReg1<typename decltype(i.src1)::reg_type>(e));
    auto src2 = GetWithConst(
        e, i.src2, GetTempReg2<typename decltype(i.src2)::reg_type>(e));
    reg_reg_fn(e, i.dest, src1, src2);
  }

  // Kept for porting familiarity with the x64 sequences; on ARM64 the
  // commutative/associative distinction doesn't buy anything.
  template <typename REG_REG_FN>
  static void EmitCommutativeBinaryOp(A64Emitter& e, const EmitArgType& i,
                                      const REG_REG_FN& reg_reg_fn) {
    EmitBinaryOp(e, i, reg_reg_fn);
  }
  template <typename REG_REG_FN>
  ...
  // Emits a compare of src1/src2 (materializing constants) and leaves the
  // result in the condition flags.
  static void EmitCompare(A64Emitter& e, const EmitArgType& i) {
    auto src1 = GetWithConst(
        e, i.src1, GetTempReg1<typename decltype(i.src1)::reg_type>(e));
    auto src2 = GetWithConst(
        e, i.src2, GetTempReg2<typename decltype(i.src2)::reg_type>(e));
